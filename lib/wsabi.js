...
wsabi.register = function (server, options, next) {
    ...
    io.sockets.on('connection', function (socket) {
        new Manager(server, socket, config);
    });

    // Register the preauth plugin to enable filtering of requests.
    plugin.ext('onPreAuth', function(req, reply) {
        var enabled = Hoek.reach(req, 'route.plugins.wsabi.enabled');
        if (enabled === false && req.headers['X-Is-Socket']) {
            return reply(Boom.badRequest('Websockets are not allowed on this route.'));
        }
